...
> ⚠️ **前置条件**：账户余额 ≥ 200 元或已购买资源包，否则无法开通。
...
2. GET  /v3/contents/generations/tasks/{id}  →  轮询状态
...
| `duration` | int | 5 | 视频秒数 [2,12]或[4,15]，-1为智能 |
| `resolution` | string | 720p/1080p | 480p/720p/1080p |
| `generate_audio` | bool | true | 是否生成音频（2.0/1.5pro） |
| `watermark` | bool | false | 是否添加AI水印 |
| `seed` | int | -1 | 随机种子[-1, 2³²-1] |
| `callback_url` | string | — | 完成回调 URL |
| `return_last_frame` | bool | false | 是否返回尾帧 |
| `execution_expires_after` | int | 172800 | 任务超时秒数 [3600,259200] |
| `priority` | int | 0 | 优先级 [0,9]，仅 2.0 |
| `safety_identifier` | string | — | 用户唯一标识 ≤64 字符 |
| `tools` | array | — | 2.0：[{"type":"web_search"}] |
